...
def get_state(state, action):
    res = state.copy() 
    if action   in (Action.UP, Action.UP_RIGHT, Action.UP_LEFT) \
            and res[0] > 0:
        res[0] -= 1
    elif action in (Action.DOWN, Action.DOWN_RIGHT, Action.DOWN_LEFT) \
            and res[0] < YBOUND:
        res[0] += 1
    if action   in (Action.RIGHT, Action.DOWN_RIGHT, Action.UP_RIGHT) \
            and res[1] < XBOUND:
        res[1] += 1
    elif action in (Action.LEFT, Action.DOWN_LEFT, Action.DOWN_RIGHT) \
            and res[1] > 0:
        res[1] -= 1

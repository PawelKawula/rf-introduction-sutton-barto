...
GAMMA, EPS, ALPHA, N_EPISODES = 0.9, 0.1, 0.1, 100000
...
class Action(IntEnum):
    UP, DOWN, RIGHT, LEFT, UP_RIGHT, UP_LEFT, DOWN_RIGHT, DOWN_LEFT = range(8)
...
@nb.njit
def get_wind(state):
    add_wind = nprand.randint(-1, 2) if GRIDWORLD[state[0], state[1]] != 0 else 0
...
def get_state(state, action):
    res = state.copy() 
    if action   in (Action.UP, Action.UP_RIGHT, Action.UP_LEFT) and res[0] > 0:
        res[0] -= 1
    elif action in (Action.DOWN, Action.DOWN_RIGHT, Action.DOWN_LEFT) and res[0] < YBOUND:
        res[0] += 1
    if action   in (Action.RIGHT, Action.DOWN_RIGHT, Action.UP_RIGHT) and res[1] < XBOUND:
        res[1] += 1
    elif action in (Action.LEFT, Action.DOWN_LEFT, Action.DOWN_RIGHT) and res[1] > 0:
        res[1] -= 1
    res[0] = min(max(res[0] - get_wind(state), 0), YBOUND)
    return res

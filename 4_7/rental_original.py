...
def save_policy():
    ...
    ax.invert_yaxis()
    plt.savefig('pi'+str(save_policy.counter)+'.svg')
    plt.close()
...
@njit
def get_new_state_indexes(state: array, action1: int, action2: int) -> float:
    return [max(0, min(MAX_CARS, state[0] + action1)), max(0, min(MAX_CARS, state[1] + action2))]

@njit
def get_v(vs: array, state: array, action: int) -> float:

    v = abs(action) * MOVE_REWARD
